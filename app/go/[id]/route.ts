...
export async function GET(request: NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const rows=await sql`
    SELECT id,slug,platform,marketplace_url,destination_url,affiliate_url,active
    FROM products
    WHERE id=${productId}
    LIMIT 1
  `;
  ...
  if(!destination){
    ...
    return NextResponse.redirect(internalUrl(request,"/"),302);
  }
  ...
  try{
    await sql`INSERT INTO clicks(product_id,destination_url,source,referrer,user_agent,clicked_at) VALUES (${productId},${destination},${source},${referrer},${userAgent},NOW())`;
  }catch(error){

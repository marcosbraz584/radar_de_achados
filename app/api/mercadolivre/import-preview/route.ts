...
type Reference = { id: string; type: "item" | "catalog_product"; source: "wid" | "path"; catalogId: string | null };
...
async function getItemDescription(itemId: string) {
  try {
    const response = await mlFetch(`https://api.mercadolibre.com/items/${itemId}/description`);
    if (!response.ok) return null;
    const data = await response.json();
    const text = typeof data?.plain_text === "string" ? data.plain_text.trim() : "";
    return text || null;
  } catch { return null; }
}
...
async function getItemPrice(itemId: string, fallbackPrice: number | null) {
  ...
  if (saleResponse.ok) {
    ...
    if (typeof data?.amount === "number") return { data: { price: data.amount, original_price: typeof data?.regular_amount === "number" ? data.regular_amount : null, currency_id: data?.currency_id || "BRL" }, status: saleResponse.status };
  }
  ...
  if (pricesResponse.ok) {
    ...
    const preferred = marketplace.find((price) => price.type === "promotion" && typeof price.amount === "number") || marketplace.find((price) => price.type === "standard" && typeof price.amount === "number") || marketplace.find((price) => typeof price.amount === "number");
    if (typeof preferred?.amount === "number") return { data: { price: preferred.amount, original_price: typeof preferred.regular_amount === "number" ? preferred.regular_amount : null, currency_id: preferred.currency_id || data?.currency_id || "BRL" }, status: pricesResponse.status };
  }
  return { data: fallbackPrice != null ? { price: fallbackPrice, original_price: null, currency_id: "BRL" } : null, status: pricesResponse.status || saleResponse.status };
}
...
async function getCatalogOffer(catalogId: string, preferredItemId?: string | null) {
  try {
    ...
    const valid = offers.filter((offer) => typeof offer.item_id === "string" && (!offer.condition || offer.condition === "new"));
    const selected = valid.find((offer) => offer.item_id === preferredItemId) || valid.filter((offer) => typeof offer.price === "number").sort((a, b) => (a.price ?? Infinity) - (b.price ?? Infinity))[0] || valid[0];
    if (!selected?.item_id) return null;
    return { itemId: selected.item_id, price: typeof selected.price === "number" ? selected.price : null, originalPrice: typeof selected.original_price === "number" ? selected.original_price : null, currencyId: selected.currency_id || "BRL" };
  } catch { return null; }
}
...
export async function GET(request: Request) {
  try {
    ...
    if (!requestedItemId && reference.catalogId) { const offer = await getCatalogOffer(reference.catalogId); if (offer) requestedItemId = offer.itemId; }
    ...
    if (!response.ok && type === "item" && (response.status === 403 || response.status === 404) && reference.catalogId) {
      id = reference.catalogId; type = "catalog_product"; response = await mlFetch(`https://api.mercadolibre.com/products/${id}`); data = await response.json(); usedCatalogFallback = response.ok;
    } else if (!response.ok && type === "item" && response.status === 404 && reference.source !== "wid") {
      response = await mlFetch(`https://api.mercadolibre.com/products/${id}`); data = await response.json(); type = "catalog_product";
    }
    ...
    const pictures = Array.isArray(data.pictures) ? (data.pictures as MercadoLivrePicture[]).map((picture) => picture?.secure_url || picture?.url || null).filter((picture): picture is string => Boolean(picture)) : [];
    const fallbackPrice = catalogOffer?.price ?? null;
    const [priceResult, description] = await Promise.all([
      requestedItemId ? getItemPrice(requestedItemId, fallbackPrice) : Promise.resolve({ data: null, status: null }),
      requestedItemId ? getItemDescription(requestedItemId) : Promise.resolve(null),
    ]);
    const salePrice = priceResult.data;
    const hasPrice = salePrice?.price != null;
    const note = hasPrice ? (usedCatalogFallback ? "Nome e imagens vieram do catálogo e o preço foi preservado da oferta selecionada/confirmado pela API de preços." : "Oferta identificada e preço atual consultado automaticamente no Mercado Livre.") : type === "catalog_product" ? "Produto de catálogo identificado, mas nenhuma oferta com preço disponível foi encontrada." : "Anúncio identificado, mas a API de preços não retornou um preço para este item.";
    return NextResponse.json({
      ...
      automation: { basic_data: true, images: pictures.length > 0, description: Boolean(description), price: hasPrice, reference_source: reference.source, item_access_status: itemAccessStatus, price_access_status: priceResult.status, catalog_fallback: usedCatalogFallback, note },
    });
  } catch (error) {

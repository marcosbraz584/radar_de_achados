...
type AvailabilityResult = { ok: boolean; quantity: number | null; status: string | null };
...
async function getAvailability(itemId: string, token: string): Promise<AvailabilityResult> {
  const response = await fetchWithToken(`https://api.mercadolibre.com/items/${encodeURIComponent(itemId)}`, token);
  if (!response.ok) return { ok: false, quantity: null, status: null };
  const data = await response.json();
  const quantity = typeof data?.available_quantity === "number" ? data.available_quantity : null;
  const marketplaceStatus = typeof data?.status === "string" ? data.status : null;
  const status = quantity === 0 ? "out_of_stock" : marketplaceStatus === "active" ? "available" : marketplaceStatus;
  return { ok: true, quantity, status };
}
...
export async function POST(request: Request) {
  ...
  const availability = await getAvailability(String(result.itemId || product.marketplace_item_id), token);

  if (!result.ok || result.price == null) {
    const status = result.status === 403 ? "restricted" : "error";
    await sql`UPDATE products SET price_sync_status=${status},price_checked_at=NOW(),last_synced_at=NOW(),available_quantity=COALESCE(${availability.quantity},available_quantity),availability_status=COALESCE(${availability.status},availability_status),availability_checked_at=CASE WHEN ${availability.ok} THEN NOW() ELSE availability_checked_at END WHERE id=${productId}`;
    return NextResponse.json({ok:result.status===403,status,pricePreserved:true,availabilityUpdated:availability.ok,availableQuantity:availability.quantity,availabilityStatus:availability.status},{status:result.status===403?200:502});
  }
  ...
  await sql`UPDATE products SET promo_price=${result.price},regular_price=COALESCE(${result.regularPrice},regular_price),marketplace_item_id=${result.itemId},price_source='automatic',price_sync_status='ok',price_checked_at=NOW(),last_synced_at=NOW(),updated_at=NOW(),available_quantity=COALESCE(${availability.quantity},available_quantity),availability_status=COALESCE(${availability.status},availability_status),availability_checked_at=CASE WHEN ${availability.ok} THEN NOW() ELSE availability_checked_at END WHERE id=${productId}`;
  return NextResponse.json({ok:true,status:"ok",price:result.price,regularPrice:result.regularPrice,historyRecorded:priceChanged,source:result.source,availabilityUpdated:availability.ok,availableQuantity:availability.quantity,availabilityStatus:availability.status});
}

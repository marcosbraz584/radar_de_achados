...
export async function POST(request: Request) {
  ...
  const offerStatus = availability.status || "available";
  await sql`UPDATE products SET promo_price=${result.price},regular_price=COALESCE(${result.regularPrice},regular_price),marketplace_item_id=${result.itemId},price_source='automatic',price_sync_status='ok',price_checked_at=NOW(),last_synced_at=NOW(),updated_at=NOW(),available_quantity=COALESCE(${availability.quantity},available_quantity),availability_status=${offerStatus},availability_checked_at=NOW() WHERE id=${productId}`;
  return NextResponse.json({ok:true,status:"ok",price:result.price,regularPrice:result.regularPrice,historyRecorded:priceChanged,source:result.source,availabilityUpdated:true,availableQuantity:availability.quantity,availabilityStatus:offerStatus});
}

...
async function getDashboardData() {
  const sql = getDb();
  const [products, categories, coupons, clicks] = await Promise.all([
    sql`SELECT COUNT(*)::int AS count FROM products`,
    sql`SELECT COUNT(*)::int AS count FROM categories`,
    sql`SELECT COUNT(*)::int AS count FROM coupons`,
    sql`SELECT COUNT(*)::int AS count FROM clicks`,
  ]);
    ...
    <main className="admin-shell">
      ...
      <section className="admin-content">
        <header className="admin-header">
          <div><p className="eyebrow">ADMINISTRAÇÃO</p><h1>Painel Radar de Achados</h1><p className="admin-subtitle">Acompanhe e gerencie sua loja em um só lugar.</p></div>
        ...
        <section className="admin-panel">
          <div><p className="eyebrow">RADAR DE ACHADOS 2.0</p><h2>Base administrativa funcionando</h2><p>O painel já está lendo os dados diretamente do PostgreSQL no Neon. Nos próximos passos, cada área do menu ganhará suas funções de cadastro e gerenciamento.</p></div>

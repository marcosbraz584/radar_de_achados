...
export default function CategoryImageUpload({defaultUrl=""}:{defaultUrl?:string}){
 ...
 async function handleFile(file:File|undefined){
  ...
  try{
   ...
   setStatus("✓ Nova imagem pronta. Clique em Salvar alterações.");
  }catch(error){
 ...
 return <>
  ...
  <label className="field field-full">
   ...
   <small>Exibição aproximada: 78 × 62 px no computador e 64 × 52 px no celular. Se deixar vazio, será usado o ícone automático.</small>
  </label>
  {imageUrl?<div className="field field-full" style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
   ...
   <button type="button" className="secondary-button" onClick={()=>{setImageUrl("");setStatus("✓ Imagem removida. Clique em Salvar alterações.");}}>Remover imagem</button>
  </div>:null}
 </>;
}

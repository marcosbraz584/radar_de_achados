...
async function getBanners() {
  const sql = getDb();
  return sql`SELECT id,title,subtitle,image_url,target_url,active,sort_order,starts_at,expires_at FROM banners ORDER BY sort_order ASC,updated_at DESC,id DESC`;
}
      ...
      <section className="admin-content">
        <header className="admin-header">
          <div>
            ...
            <h1>Banners</h1>
            <p className="admin-subtitle">Gerencie as artes promocionais e os destinos dos banners da loja.</p>
          </div>
        </header>
        ...
        <section className="form-card">
          <h2>Novo banner</h2>
          <div style={{ marginTop: 10, marginBottom: 4, padding: "12px 14px", borderRadius: 10, background: "#eff6ff", border: "1px solid #bfdbfe", color: "#1e3a8a" }}>
            <strong>Dimensão recomendada: {BANNER_SIZE}</strong>
            <br />
            <small>Use essa proporção para evitar cortes ou deformações no banner principal da loja.</small>
          </div>

          <form action={createBanner} className="form-grid two-columns" style={{ marginTop: 16 }}>
            <label className="field">
              <span>Título</span>
              <input name="title" placeholder="Ex.: Ofertas da semana" />
            </label>
            <label className="field">
              <span>Ordem</span>
              <input name="sort_order" type="number" min="0" defaultValue="9999" />
            </label>
            ...
            <div className="field field-full">
              <button className="primary-button" type="submit">+ Criar banner</button>
            </div>
          </form>
        </section>

        <section className="admin-panel products-panel" style={{ marginTop: 20 }}>
          <div className="products-toolbar">
            <div><strong>{banners.length}</strong> banner{banners.length === 1 ? "" : "s"} cadastrado{banners.length === 1 ? "" : "s"}</div>
          ...
          {banners.length === 0 ? (
            <div className="empty-state">
              ...
              <p>Crie o primeiro banner acima.</p>
            </div>
          ) : (
            <div className="product-list">
              {banners.map((b) => (
                <article className="product-row" key={b.id}>
                  <div style={{ display: "flex", gap: 16, alignItems: "center", minWidth: 0 }}>
                    <img src={b.image_url} alt={b.title || "Banner"} style={{ width: 150, height: 60, objectFit: "cover", borderRadius: 10, border: "1px solid #e2e8f0" }} />
                    <div className="product-row-info">
                      <strong>{b.title || "Banner sem título"}</strong>
                      <small>{b.subtitle || "Sem subtítulo"}</small>
                      <small>Imagem: {BANNER_SIZE} • Ordem: {b.sort_order} • {b.active ? "Ativo" : "Inativo"} • Validade: {formatDate(b.expires_at)}</small>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
                    <a className="secondary-button" href={`/admin/banners/${b.id}/editar`}>Editar</a>
                    <form action={toggleBanner}>
                      <input type="hidden" name="id" value={b.id} />
                      <button className="secondary-button" type="submit">{b.active ? "Desativar" : "Ativar"}</button>
                    </form>
                    <ExcluirBannerButton id={Number(b.id)} action={deleteBanner} />
                  </div>
                </article>
              ))}
            </div>

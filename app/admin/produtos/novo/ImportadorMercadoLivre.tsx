...
export default function ImportadorMercadoLivre() {
  ...
  async function searchProducts() {
    ...
    try {
      ...
      if (!data.ok) {
        setSearchError(data.error || "Não foi possível pesquisar agora.");
        return;
      }
      setResults(Array.isArray(data.results) ? data.results : []);
      if (!data.results?.length) setSearchError("Nenhum produto encontrado para essa busca.");
    } catch {
      setSearchError("Não foi possível pesquisar o Mercado Livre agora.");
    } finally {
      setSearching(false);
    }
  }
    ...
    <section className="form-card ml-import-card">
      <div className="ml-import-heading">
        <div>
          ...
          <h2>Buscar no Mercado Livre</h2>
          <p className="admin-subtitle">Pesquise pelo nome do produto ou cole um link do Mercado Livre.</p>
        </div>
        <span className="ml-badge">Mercado Livre conectado</span>
      </div>

      <div style={{ display: "flex", gap: 8, margin: "16px 0" }}>
        <button type="button" style={tabStyle(mode === "search")} onClick={() => setMode("search")}>Pesquisar por nome</button>
        <button type="button" style={tabStyle(mode === "link")} onClick={() => setMode("link")}>Colar link</button>
      </div>
      ...
      {mode === "search" ? (
        <>
          ...
          {searchError ? <div className="ml-result ml-error"><strong>{searchError}</strong></div> : null}
        ...
        <div className="ml-import-row">
          ...
          <button type="button" className="primary-button" onClick={() => importValue(link)} disabled={loading || !link.trim()}>
      ...
      {preview?.ok && preview.product ? (
        <div className="ml-result ml-success">

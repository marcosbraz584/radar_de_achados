...
type ProductRow={id:number;name:string;slug:string;description:string|null;short_description:string|null;regular_price:number|string|null;promo_price:number|string|null;destination_url:string|null;affiliate_url:string|null;platform:string|null;category_name:string|null;category_slug:string|null;sale_mode:string|null;product_type:string|null;sku:string|null;stock_quantity:number|string|null;store_name:string|null};
...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
 ...
 const rows=await sql`SELECT p.id,p.name,p.slug,p.description,p.short_description,p.regular_price,p.promo_price,p.destination_url,p.affiliate_url,p.platform,p.sale_mode,p.product_type,p.sku,p.stock_quantity,c.name category_name,c.slug category_slug,st.store_name FROM products p LEFT JOIN categories c ON c.id=p.category_id LEFT JOIN seller_stores st ON st.seller_id=p.seller_id WHERE (p.slug=${slug} OR (p.sale_mode='OWN' AND p.id::text=${slug})) AND p.active=TRUE AND (p.sale_mode<>'OWN' OR p.approval_status='approved') LIMIT 1`;
  ...
  <div className="pd-wrap">
   ...
   <section className="pd-shell">
    ...
    <div className="pd-info">
     ...
     <div className="pd-source">{own?<>Vendido por <strong>{product.store_name||"Vendedor SHILMASTORE"}</strong>{product.sku?<> • SKU: {product.sku}</>:null}</>:<>Oferta selecionada na {platformLabel(product.platform)||"plataforma parceira"}</>}</div>
     ...
     {own?<div className="pd-benefits"><div className="pd-benefit"><b>{product.product_type==='DIGITAL'?"Produto digital":"Produto físico"}</b>SHILMASTORE</div><div className="pd-benefit"><b>Vendedor aprovado</b>Marketplace</div><div className="pd-benefit"><b>Compra interna</b>Na SHILMASTORE</div></div>:<div className="pd-benefits"><div className="pd-benefit"><b>Compra externa</b>Finalização no parceiro</div><div className="pd-benefit"><b>Oferta verificada</b>Produto selecionado</div><div className="pd-benefit"><b>Link seguro</b>Redirecionamento direto</div></div>}
    </div>
    <aside className="pd-buybox">
     {currentPrice?<div className="pd-buy-price">{currentPrice}</div>:null}
     {own?<><div style={{fontSize:13,color:"#475569"}}>Disponibilidade</div><div className="pd-stock" style={{color:inStock?"#008f39":"#b42318"}}>{inStock?"Produto disponível":"Produto sem estoque"}</div><button className="pd-button" disabled={!inStock}>Adicionar ao carrinho</button><div className="pd-safe">O carrinho deste produto será conectado ao fluxo de compra da SHILMASTORE na próxima etapa.</div></>:<><div style={{fontSize:13,color:"#475569"}}>Oferta disponível em</div><div className="pd-stock">{platformLabel(product.platform)||"Loja parceira"}</div>{destination?<a className="pd-button" href={`/go/${product.id}?source=product-page`} target="_blank" rel="nofollow sponsored noopener noreferrer">Ver oferta na loja ↗</a>:<div style={{padding:14,background:"#fff4d6",borderRadius:9}}>Link da oferta ainda não cadastrado.</div>}<div className="pd-safe">Você será direcionado para a plataforma parceira, onde poderá conferir frete, estoque, condições de pagamento e concluir a compra.</div></>}
    </aside>
   </section>
   {(product.description||product.short_description)?<section className="pd-description"><h2>Detalhes do produto</h2><div className="pd-description-text">{product.description||product.short_description}</div></section>:null}

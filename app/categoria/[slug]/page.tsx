...
type Product={id:number;slug:string;name:string;regular_price:string|null;promo_price:string|null;platform:string|null;image_url:string|null};
...
export default async function CategoryPage({params}:{params:Promise<{slug:string}>}){
 ...
 return <main style={{minHeight:"100vh",background:"#f5f5f5",color:"#172554"}}>
  ...
  <section className="cat-wrap" style={{maxWidth:1200,margin:"auto",padding:"32px 22px 55px"}}><h1 style={{margin:"0 0 5px",fontSize:"clamp(27px,4vw,38px)"}}>{category.name}</h1><p style={{margin:"0 0 4px",color:"#64748b"}}>{products.length} produto{products.length===1?"":"s"} encontrado{products.length===1?"":"s"}</p><p className="cat-hint">Clique no produto para ver todos os detalhes e acessar a oferta.</p>
  {products.length===0?<div style={{background:"white",padding:35,borderRadius:12,marginTop:20}}>Ainda não há produtos nesta categoria.</div>:<div className="cat-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(205px,1fr))",gap:16,marginTop:18}}>{products.map(p=>{const price=money(p.promo_price)||money(p.regular_price);return <article className="cat-card" key={p.id} style={{background:"white",borderRadius:12,overflow:"hidden",boxShadow:"0 2px 10px #0000000d"}}><a className="cat-card-link" href={`/produto/${encodeURIComponent(p.slug)}`}><div className="cat-img">{p.image_url?<img src={p.image_url} alt={p.name}/>:<span style={{color:"#94a3b8"}}>Sem imagem</span>}</div><div className="cat-body"><div className="cat-name">{p.name}</div><strong className="cat-price">{price||"Consulte"}</strong><small className="cat-platform">{platformLabel(p.platform)}</small><span className="cat-button">Ver produto</span></div></a></article>})}</div>}
  </section>
 </main>;
}

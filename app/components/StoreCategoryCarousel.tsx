...
export default function StoreCategoryCarousel({categories}:{categories:Category[]}){
  ...
  useEffect(()=>{let alive=true;fetch("/api/categories/images").then(r=>r.ok?r.json():null).then(data=>{if(alive&&data?.images)setCustomImages(data.images)}).catch(()=>{});return()=>{alive=false}},[]);
  ...
  return <section className="sh-category-carousel" aria-label="Categorias em destaque">
    ...
    <div className="sh-category-carousel-wrap">
      ...
      <div className="sh-category-track" ref={trackRef}>
        {categories.map(c=>{const image=customImages[c.id];return <a className="sh-category-tile" key={c.id} href={`/categoria/${encodeURIComponent(c.slug)}`}>
          <div className="sh-category-thumb" aria-hidden="true">{image?<img src={image} alt=""/>:<span style={{fontSize:36,lineHeight:1}}>{categoryIcon(c.name)}</span>}</div>

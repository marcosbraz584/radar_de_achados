...
async function createOrder(){
 ...
 const items=await sql`
  SELECT ci.quantity,p.id product_id,p.name,p.product_type,p.regular_price,p.promo_price,p.seller_id
  FROM carts c JOIN cart_items ci ON ci.cart_id=c.id JOIN products p ON p.id=ci.product_id
  WHERE c.user_id=${Number(user.id)} AND p.active=TRUE AND p.approval_status='approved' ORDER BY ci.created_at
 `;
